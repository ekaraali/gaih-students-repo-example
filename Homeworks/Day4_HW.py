...
def prime_number(number):
    if (number == 1):
      ...
      for i in range(2, number):
          ...
          #if any number smaller than the current number is divisible by the current number, than return False
         if (number%i == 0):
            return False
      #if none of number smaller than the current number is divisible by the current number, than return True
      return True
